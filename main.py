...
from models import Basic_Details, Education, Location, Project_Details, Skills, Social_Media, Work_Experience
import json

db = Session()

#****************List all resume*******************
async def get_all_resume(request):
    basic_details_from_database = db.query(Basic_Details).order_by(Basic_Details.basic_details_id.desc()).all()
    basic_details_content = [{k:v for k, v in vars(basic_detail).items() if k != '_sa_instance_state' } for basic_detail in basic_details_from_database]
    #converting type of applied_date to string
    basic_details_content = [{k:str(v) if k=='applied_date' else v for k, v in content.items() } for content in basic_details_content]
    return JSONResponse({'data': basic_details_content},status_code=200)


# ********************Search by name or email***************************
async def search_by_name_or_email(request:Request):
    search_by_condition=request.path_params.get('name_or_email')
    basic_details_from_database=db.query(Basic_Details).filter((Basic_Details.name == search_by_condition) | (Basic_Details.email == search_by_condition)).all()
    basic_details_content = [{k:v for k,v in vars(basic_details).items() if k!= '_sa_instance_state'}
    for basic_details in basic_details_from_database]
    content=[{k:str(v) if k=='applied_date' else v for k, v in content.items() } for content in basic_details_content]
...
async def delete_resume(request:Request):
    ...
    data_to_be_deleted=db.query(Basic_Details).filter(Basic_Details.basic_details_id == id_to_be_deleted)
...
def get_basic_details(id):
    basic_details_from_database=db.query(Basic_Details).filter(Basic_Details.basic_details_id == id).all()
    database_content = [{k:v for k,v in vars(basic_detail).items() if k!= '_sa_instance_state'}
            for basic_detail in basic_details_from_database]
    database_content=[{k:str(v) if k=='applied_date' else v for k, v in content.items() } for content in database_content]
    return database_content

def get_location_details(id):
    location_details_from_database=db.query(Location).filter(Location.basic_details_id == id).all()
    location_content = [{k:v for k,v in vars(location).items() if k!= '_sa_instance_state'}
            for location in location_details_from_database]
    return location_content

def get_education_details(id):
    education_details_from_database=db.query(Education).filter(Education.basic_details_id == id).all()
    education_content = [{k:v for k,v in vars(education).items() if k!= '_sa_instance_state'}   for education in education_details_from_database]
    education_content = [{k:str(v) if k in ['start_date', 'end_date'] else v for k, v in cont.items() } for cont in education_content]
    return education_content

def get_work_experience_details(id):
    work_experience_details_from_database=db.query(Work_Experience).filter(Work_Experience.basic_details_id == id).all()
    experience_content = [{k:v for k,v in vars(experience).items() if k!= '_sa_instance_state'}
            for experience in work_experience_details_from_database]
    experience_content = [{k:str(v) if k in ['start_date', 'end_date'] else v for k, v in cont.items() } for cont in experience_content]
    return experience_content

def get_project_details(id):
    project_details_from_database=db.query(Project_Details).filter(Project_Details.basic_details_id == id).all()
    project_content = [{k:v for k,v in vars(project).items() if k!= '_sa_instance_state'}
        for project in project_details_from_database]
    return project_content

def get_skill_details(id):
    skill_details_from_database=db.query(Skills).filter(Skills.basic_details_id == id).all()
    skill_content = [{k:v for k,v in vars(skill).items() if k!= '_sa_instance_state'}
            for skill in skill_details_from_database]
    return skill_content

def get_social_media_details(id):
    social_media_details_from_database=db.query(Social_Media).filter(Social_Media.basic_details_id == id).all()
    social_media_content = [{k:v for k,v in vars(social_media).items() if k!= '_sa_instance_state'}
...
async def get_individual_resume(request:Request):
    ...
    if request.method == "GET":
        return JSONResponse(
            {
                "basic details":get_basic_details(id),
                "location details" : get_location_details(id),
                "education details": get_education_details(id),
                "work experience details":get_work_experience_details(id),
                "project details":get_project_details(id),
                "skill details" :get_skill_details(id),
                "social media details":get_social_media_details(id)
            }
        )
...
async def update_individual_resume(request:Request):
    ...
    if request.method == "PUT":
        ...
        basic=update_data.pop("basic details")
        location=update_data.pop("location details")
        education=update_data.pop("education details")
        experience=update_data.pop("work experience details")
        project=update_data.pop("project details")
        skills=update_data.pop("skill details")
        social_media=update_data.pop("social media details")

        try:
            for bas in basic:
                basic_details_db=db.query(Basic_Details).filter(Basic_Details.basic_details_id == id)
                basic_details_db.update(bas,synchronize_session=False)
                db.commit()

            for  loc in location:
                if "location_id" in loc.keys():
                    location_details_db=db.query(Location).filter((Location.basic_details_id == id) & (Location.location_id == loc['location_id']))
            ...
            house_name_db=db.query(Location.address).filter((Location.basic_details_id == id)).all()
            ...
            for  edu in education:
                if "education_details_id" in edu.keys():
                    education_details_db=db.query(Education).filter((Education.basic_details_id == id) & (Education.education_details_id==edu['education_details_id']))
            ...
            qualification_db=db.query(Education.qualification).filter((Education.basic_details_id == id)).all()
            ...
            for q in qualification_from_database:
                if q not in qualification:
                    data_delete=db.query(Education).filter(Education.qualification == q)
                    data_delete.delete()
                    db.commit()

            for exp in experience:
                if "experience_details_id" in exp.keys():
                    experience_details_db=db.query(Work_Experience).filter((Work_Experience.basic_details_id == id) & (Work_Experience.experience_details_id==exp['experience_details_id']))
            ...
            job_roles_db=db.query(Work_Experience.job_role).filter((Work_Experience.basic_details_id == id)).all()
            job_roles_from_database = [lis[0] for lis in job_roles_db]
            for job_role in job_roles_from_database:
                if job_role not in job_roles:
                    data_delete=db.query(Work_Experience).filter(Work_Experience.job_role == job_role)
                    data_delete.delete()
                    db.commit()

            for pro in project:
                if "project_details_id" in pro.keys():
                    project_details_db=db.query(Project_Details).filter((Project_Details.basic_details_id == id) & (Project_Details.project_details_id == pro['project_details_id']))
            ...
            project_titles_db=db.query(Project_Details.project_title).filter((Project_Details.basic_details_id == id)).all()
            ...
            for project_title in project_titles_from_database:
                if project_title not in project_titles:
                    data_delete=db.query(Project_Details).filter(Project_Details.project_title == project_title)
                    data_delete.delete()
                    db.commit()

            skill_names_in_data_to_be_edited = [ sub['skill_name'] for sub in skills]
            skills_name_db=db.query(Skills.skill_name).filter((Skills.basic_details_id == id)).all()
                ...
                if skill_name not in skill_names_from_database:
                    for  skill in skills:
                        if "skills_id" in skill.keys():
                            print("edit")
                            skills_details_db=db.query(Skills).filter((Skills.basic_details_id == id) & (Skills.skills_id==skill['skills_id']))
            ...
            for skill_name in skill_names_from_database:
                if skill_name not in skill_names_in_data_to_be_edited:
                    data_delete=db.query(Skills).filter(Skills.skill_name == skill_name)
            ...
            for soc in social_media:
                if "social_media_id" in soc.keys():
                    social_media_db=db.query(Social_Media).filter((Social_Media.basic_details_id == id) & (Social_Media.social_media_id == soc['social_media_id']))
            ...
            social_media_urls_db=db.query(Social_Media.url).filter((Social_Media.basic_details_id == id)).all()
...
routes=[
    ...
    Route('/resume/{name_or_email:str}/', endpoint=search_by_name_or_email),
    Route('/individual-resume/{applicant_id:int}/', endpoint=get_individual_resume, methods=["GET"]),
    Route('/new-resume',endpoint=create_new_resume, methods=['GET', 'POST']),
    Route('/update-individual-resume/{applicant_id:int}/', endpoint=update_individual_resume, methods=["PUT"]),
    Route('/delete-resume/{applicant_id:int}/',endpoint=delete_resume,methods=["DELETE"]),
]
